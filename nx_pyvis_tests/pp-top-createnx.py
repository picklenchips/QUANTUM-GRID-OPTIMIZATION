...
def get_edge_table(net, table_name: str, include_edges) -> pd.DataFrame:
    """
    Returns the table for the specified name according to parameter include_edges. If it is True,
    the whole table is returned. if False, None is returned. If it is an iterable, only the included
    indices are chosen.
    ...
    if isinstance(include_edges, bool):
        ...
        # uses pandas dataframe .loc to get correct indices.
...
def init_par(tab: pd.DataFrame, calc_branch_impedances=False) -> tuple[np.ndarray, np.ndarray] | tuple[np.ndarray, np.ndarray, pd.DataFrame]:
    n = tab.shape[0]
